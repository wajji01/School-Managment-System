...
class student_managment {
  ...
  enroll_std(std_id: number, course: string) {
  ...
  std_view_balance(std_id: number) {
    let std = this.find_std(std_id);
    if (std) {
      std.view_balance();
    } else {
  ...
  std_pay_fees(std_id: number, amount: number) {
    let std = this.find_std(std_id);
    if (std) {
      std.pay_fees(amount);
    } else {
